...
describe("GET /api/admin/formularios/:id", () => {
  ...
  test("200 y devuelve el formulario con sus preguntas", async () => {
...
describe("PATCH /api/admin/formularios/:id", () => {
  ...
  test("200 y actualiza solo metadatos si no se envían preguntas", async () => {
    const res = await patch(mock.formEmociones, { titulo: "Bienestar emocional v2" });
    expect(res.status).toBe(200);

    const preguntasAntes = await sql`SELECT id FROM pregunta WHERE formulario_id = ${mock.formEmociones}`;
    expect(preguntasAntes).toHaveLength(2); // no se tocaron
  });

  test("200 y reemplaza el set de preguntas cuando se envía uno nuevo", async () => {
    // Formulario recién creado, sin respuestas registradas todavía
    const formulario = await createFormulario();
    const res = await patch(formulario.id, {
      preguntas: [
        { texto: "Nueva pregunta única", opcionesRespuesta: [{ valor: 1, texto: "Sí" }, { valor: 2, texto: "No" }] },
      ],
    });
    expect(res.status).toBe(200);

    const preguntas = await sql`SELECT texto FROM pregunta WHERE formulario_id = ${formulario.id}`;
  ...
  test("409 si alguna de las preguntas actuales ya tiene respuestas registradas", async () => {
    ...
    const res = await patch(mock.formEmociones, {
      preguntas: [
        { texto: "Pregunta nueva", opcionesRespuesta: [{ valor: 1, texto: "Sí" }, { valor: 2, texto: "No" }] },
      ],
    });
    expect(res.status).toBe(409);

    const preguntas = await sql`SELECT id FROM pregunta WHERE formulario_id = ${mock.formEmociones}`;
    expect(preguntas).toHaveLength(2); // no se tocaron
  });
});
