...
**Catálogo de imágenes predefinidas** (`pregunta.imagen_url` / `seccion.instruccion_imagen_url` — guardan el key del bucket, no una URL, ver "Presigned URLs" más abajo): en vez de que el admin pegue una URL a mano, elige de una galería servida por `GET /api/admin/imagenes?carpeta=<prefijo>` (`src/routes/admin/imagenes.ts` → `storageService.listFiles`), consumida por `src/components/SelectorImagen.tsx`. El catálogo vive en dos carpetas **planas y compartidas** entre todos los formularios — `assets/instrucciones/` y `assets/preguntas/`, sin subcarpeta por set — y son las únicas dos que acepta el `CARPETAS_PERMITIDAS` de `imagenes.ts`; no es un allowlist pensado para crecer con un prefijo nuevo por cada set de imágenes. Subir imágenes nuevas a esas mismas carpetas: poner los `.png` en `src/assets/<set>/instrucciones` y `src/assets/<set>/preguntas`, y correr algo como `scripts/upload-form-emociones-assets.ts` (o `bun run seed:imagenes` para el set de `form_emociones` ya armado) apuntando a esos dos prefijos. Las imágenes en `src/assets/` **no se importan como módulos ES** (a diferencia del logo) — son ilustraciones de varios MB cada una (~120MB el set de `form_emociones`), empaquetarlas en el bundle de React sería contraproducente; viven en el bucket igual que los PDFs de reportes.
