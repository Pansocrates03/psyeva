...
**Catálogo de imágenes predefinidas** (`pregunta.imagen_url` / `seccion.instruccion_imagen_url`): en vez de que el admin pegue una URL a mano, elige de una galería servida por `GET /api/admin/imagenes?carpeta=<prefijo>` (`src/routes/admin/imagenes.ts` → `storageService.listFiles`), consumida por `src/components/SelectorImagen.tsx`. `carpeta` está restringida a un allowlist en el propio route — agregar un set de imágenes nuevo significa: (1) poner los `.png` en `src/assets/<set>/...`, (2) subirlos al bucket bajo `assets/<set>/...` (ver `scripts/upload-form-emociones-assets.ts` como ejemplo, o `bun run seed:imagenes` para el set de `form_emociones` ya armado), (3) agregar el prefijo al `CARPETAS_PERMITIDAS` de `imagenes.ts`. Las imágenes en `src/assets/` **no se importan como módulos ES** (a diferencia del logo) — son ilustraciones de varios MB cada una (~120MB el set de `form_emociones`), empaquetarlas en el bundle de React sería contraproducente; viven en el bucket igual que los PDFs de reportes.
